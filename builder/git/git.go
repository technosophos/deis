...
func Receive(c cookoo.Context, p *cookoo.Params) (interface{}, cookoo.Interrupt) {
	...
	if err != nil {
		c.Logf("warn", "Illegal repo name: %s.", err)
	...
	if _, err := createRepo(c, filepath.Join(gitHome, repo), gitHome); err != nil {
		c.Logf("info", "Did not create new repo: %s", err)
	}
	c.Logf("info", "git-shell -c '%s %s'", operation, repo)
	cmd := exec.Command("git-shell", "-c", fmt.Sprintf("%s '%s'", operation, repo))

	var errbuff bytes.Buffer
	// errwriter := io.MultiWriter(channel.Stderr(), &errbuff)
	...
	if err := cmd.Start(); err != nil {
		c.Logf("warn", "Failed git receive immediately: %s %s", err, errbuff.Bytes())
		return nil, err
	}
	fmt.Printf("Waiting for command to run.\n")
	done.Wait()
	fmt.Printf("Waiting for cleanup.\n")
	if err := cmd.Wait(); err != nil {
		c.Logf("error", "Error on command: %s %s", err, errbuff.Bytes())
		return nil, err
	}
	if errbuff.Len() > 0 {
		c.Logf("warn", "Unreported error: %s", errbuff.Bytes())
	}
	c.Logf("info", "Command complete.\n")
...
func createRepo(c cookoo.Context, repoPath, gitHome string) (bool, error) {
	...
	if fi, err := os.Stat(repoPath); err == nil && fi.IsDir() {
		// Nothing to do.
		c.Logf("info", "Directory %s already exists.", repoPath)
		return false, nil
	} else if os.IsNotExist(err) {

		c.Logf("info", "Creating new directory at %s", repoPath)
		// Create directory
		if err := os.MkdirAll(repoPath, 0755); err != nil {
			c.Logf("warn", "Failed to create repository: %s", err)
			return false, err
		}
		cmd := exec.Command("git", "init", "--bare")
		cmd.Dir = repoPath
		if out, err := cmd.CombinedOutput(); err != nil {
			c.Logf("warn", "git init output: %s", out)
			return false, err
		}
